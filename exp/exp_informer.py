...
class Exp_Informer(Exp_Basic):
    def __init__(self, args):
        ...
        self.test_losses = []   # 存储测试损失
        self.training_metrics = []  # 存储每个epoch的评估指标
    ...
    class MixedLoss(nn.Module):
        def __init__(self, mse_weight=0.5, huber_weight=0.5):
            super().__init__()
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(train_loader):
                ...
                epoch_trues.append(true.detach().cpu().numpy())
            ...
            epoch_trues = np.array(epoch_trues)
            epoch_preds = epoch_preds.reshape(-1, epoch_preds.shape[-2], epoch_preds.shape[-1])
            epoch_trues = epoch_trues.reshape(-1, epoch_trues.shape[-2], epoch_trues.shape[-1])
            
            # 计算当前epoch的评估指标
            mae, mse, rmse, mape, mspe, rse, corr, spearman_corr_val, euclidean_dist_val, dtw_dist_val, r2_score, accuracy = metric(epoch_preds, epoch_trues)
            self.training_metrics.append([mae, mse, rmse, mape, mspe, rse, corr, euclidean_dist_val, dtw_dist_val, r2_score, accuracy])
            ...
            if self.use_wandb and wandb.run:
                metrics = {
                    ...
                    "euclidean_distance": euclidean_dist_val,
                    "dtw_distance": dtw_dist_val,
                    "r2_score": r2_score,
                    "accuracy": accuracy
                }
                try:
        ...
        # # 保存训练过程中的评估指标
        # training_metrics = np.array(self.training_metrics)
        #
        # # 保存训练指标到文件
        # folder_path = './results/' + setting + '/'
        # if not os.path.exists(folder_path):
        #     os.makedirs(folder_path)
        # np.save(folder_path+'training_metrics.npy', training_metrics)
        return self.model
    ...
    def test(self, setting):
        ...
        # 计算并记录测试指标
        mae, mse, rmse, mape, mspe, rse, corr, spearman_corr_val, euclidean_dist_val, dtw_dist_val, r2_score, accuracy = metric(preds, trues)
        ...
        # 同时保存numpy格式
        test_metrics_array = np.array([mae, mse, rmse, mape, mspe, rse,
                                     r2_score])
        np.save(folder_path+'test_metrics.npy', test_metrics_array)
